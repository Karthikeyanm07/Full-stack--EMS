...
// Auto Check-out for employees. If user forgot to checkout we need remind
// them and Automatically check-out after work time that's for this fn.
const autoCheckOut = inngest.createFunction(
	...
	async ({ event, step }) => {
		...
		if (!attendance?.checkOut) {
			...
			// send reminder email -> using node mailer
			await sendEmail({
				to: employee.email,
...
const attendanceReminderCron = inngest.createFunction(
	...
	async ({ step }) => {
		...
		return {
			...
			checkedInIds: checkedInIds.length,
